  ...
  float nms_th)
{
  ...
  // computing prediction for each position of the prediction map
  for(int height_i=0;height_i<feature_height;height_i++)
  {
    for(int width_i=0;width_i<feature_width;width_i++)
    {
      ...
      // the coordinates (m_x, my)(with 1 meter as the unit) of 3D box center in the ego coordiantes.
      // 
      ...
      // the yaw angle of the detected 3D box.
      ...
      // the length and width of the detected 3D box.
      ...
      // the adaptive object score threshold based on the distance???. 
      ...
      if(m_y>180)
        m_obj_th=obj_th*0.3;
      // filter the detections with score below the threshold.
      if(is_obj>m_obj_th)
      {
        // compute the label cls_num of the detected 3D box based on the predicted scores
        // for classes 0, 1, 2.
        int cls_num=0;
        ...
        box one_obj;
        one_obj.ry = theta; //yaw angle.
        one_obj.l = reg_l;
        one_obj.w = reg_w;
        one_obj.x = m_x;
        one_obj.z = m_y;
        one_obj.cls_num=cls_num;
        one_obj.is_obj=is_obj; // the objectness score.
        one_obj.h = reg_h;
        one_obj.y = m_z;
...
/*
*/
py::array_t<float> data2voxel(py::array_t<float> &pclist, int HEIGHT,int WIDTH, int CHANNELS, \
float X_MIN,float Y_MIN,float Z_MIN,  float X_MAX,float Y_MAX,float Z_MAX,\
float DX, float DY, float DZ, float overlap)
{
  auto points_clouds = pclist.unchecked<2>();

  auto data = py::array_t<float>(HEIGHT*WIDTH*CHANNELS);
  data.resize({HEIGHT, WIDTH, CHANNELS});
  py::buffer_info buf_data = data.request();
  float* ptr_data = (float*)buf_data.ptr;
  //cout << "points_clouds.shape(0) is : " << points_clouds.shape(0) << endl;
  for(int i=0; i < HEIGHT*WIDTH*CHANNELS; i++)
  {
    ptr_data[i] = 0;
  }

  for(int i=0; i < points_clouds.shape(0); i++)
  {
    float X = points_clouds(i, 0);
    float Y = points_clouds(i, 1);
    float Z = points_clouds(i, 2);
...
// the conversion between python fucntion and C++ function.
